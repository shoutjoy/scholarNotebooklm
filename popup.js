...
const SCHOLAR_PROMPTS = {
  slideTools: {
    ...
    step2: (from, to) => `- 대상: 슬라이드 ${from}~${to}번 주제.\n- 지시 사항:\n  1. 해당 구간의 내용을 설명할 때, 본문에 등장하는 모든 **고유 명사(학자 이름, 모델명)**와 **통계 수치(백분율, 시간 등)**를 하나도 빠짐없이 포함하라. 2. (매우 중요) "더 이상 설명할 것이 없다"고 판단될 때까지 본문의 예시와 설명을 최대한 길게 서술하라. 3. 각 슬라이드에 들어갈 시각적 요소(그래프의 x-y축 내용, 도표의 항목)를 본문의 텍스트를 근거로 복원하여 설명하라. \n- 출력 원칙: 각 문장 뒤에 해당 정보가 있는 소스 번호를 반드시 기재하라.`,
...
async function loadState() {
  ...
  try {
    ...
    const tomdType = data[STORAGE_KEYS.tomdOpenType] || 'internal';
...
function setLeftPanelWidth(pct) {
  ...
  if (main) main.style.setProperty('--left-panel-width', pct + '%');
}
...
function initPanelSplitter() {
  ...
  splitter.addEventListener('pointermove', (e) => {
    ...
    const newW = Math.max(80, Math.min(main.offsetWidth - 120, startW + dx));
    const pct = Math.round((newW / main.offsetWidth) * 100);
    const clamped = Math.max(LEFT_PANEL_MIN_PCT, Math.min(LEFT_PANEL_MAX_PCT, pct));
  ...
  try {
    if (typeof chrome !== 'undefined' && chrome.tabs?.query) {
      ...
      if (tab?.url?.includes('notebooklm.google.com')) {
        const res = await chrome.tabs.sendMessage(tab.id, { action: 'pasteAndExecute', text, runImmediately });
        if (res?.ok) {
...
async function initiateScrap() {
  try {
    ...
    let text = '';
    if (tab?.url?.includes('notebooklm.google.com')) {
      const res = await chrome.tabs.sendMessage(tab.id, { action: 'getLastMessageText' });
      if (res?.text && res.text.trim().length > 0) {
        text = res.text;
      }
      if (!text) {
        const clickRes = await chrome.tabs.sendMessage(tab.id, { action: 'clickCopyButton' });
...
async function sendToMD() {
  ...
  const internalUrl = extensionBaseUrl ? extensionBaseUrl + 'vieweditor/index.html' : url;
  if (storage) {
    try {
...
async function saveCurrentScrap() {
  const content = getCurrentScrapContent();
  if (!content) { showMessage('저장할 스크랩이 없습니다.', 'x'); return; }
  scrappedContent = content;
  await appendAccumulatedScrap(content);
  showMessage('저장 후 누적 목록에 추가했습니다.');
}
...
async function sendAccumulatedToMD() {
  ...
  if (storage) {
    try {
      await storage.set({ [STORAGE_KEYS.toMDPaste]: content });
      const data = await storage.get(STORAGE_KEYS.tomdOpenType);
      if (data[STORAGE_KEYS.tomdOpenType] !== 'external') url = extensionBaseUrl ? extensionBaseUrl + 'vieweditor/index.html' : url;
    } catch (e) {
      if (String(e).includes('Extension context invalidated')) {
        ensureExtensionContextOrNotify();
        return false;
